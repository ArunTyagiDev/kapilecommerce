            ...
            <div class="row">
                ...
                <div class="col-md-4">
                    ...
                    <div class="mb-3 border-top pt-3">
                        <div class="form-check">
                            <input class="form-check-input" type="checkbox" id="is_customizable" name="is_customizable" value="1" {{ old('is_customizable', $product->is_customizable) ? 'checked' : '' }}>
                        ...
                        <select class="form-select form-select-sm mt-2" name="customization_type">
                            <option value="photo" {{ old('customization_type', $product->customization_type ?? 'photo') === 'photo' ? 'selected' : '' }}>Photo upload (acrylic, clocks…)</option>
                            <option value="text_sticker" {{ old('customization_type', $product->customization_type) === 'text_sticker' ? 'selected' : '' }}>School name sticker (text details)</option>
                        </select>
                        <input type="text" class="form-control form-control-sm mt-2" name="shape_label" value="{{ old('shape_label', $product->shape_label) }}" placeholder="Portrait, Landscape, Circle…">
                        <select class="form-select form-select-sm mt-2" name="style_filter">
                            ...
                            @foreach(\App\Services\OmgsCatalogService::STYLE_FILTERS as $key => $label)
                                @if($key !== 'all')
                                <option value="{{ $key }}" {{ old('style_filter', $product->style_filter) === $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endif
                            @endforeach
                        </select>
                        <input type="url" class="form-control form-control-sm mt-2" name="omgs_source_url" value="{{ old('omgs_source_url', $product->omgs_source_url) }}" placeholder="OMGS product URL">
                        <div class="form-check mt-2">
                            <input class="form-check-input" type="checkbox" id="allows_cod" name="allows_cod" value="1" {{ old('allows_cod', $product->allows_cod) ? 'checked' : '' }}>
                            <label class="form-check-label" for="allows_cod">Allow COD</label>
                        </div>
                    </div>
                </div>
            </div>
